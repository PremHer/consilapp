  ...
  if (error || !expediente) {
    return (
      <div className="max-w-3xl mx-auto pt-xl px-md text-center">
        <ShieldAlert size={64} className="text-error mx-auto mb-md" />
        <h2 className="text-error font-headline-lg mb-sm">Expediente no encontrado</h2>
        <p className="text-body-lg text-on-surface-variant mb-xl">No pudimos encontrar el expediente #{numero}. Verifique el enlace proporcionado.</p>
        <Link to="/" className="bg-primary text-on-primary px-lg py-sm rounded-lg font-label-lg hover:opacity-90 transition-all shadow-md">
          Volver al Inicio
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto pt-xl px-md pb-xl">
      <div className="mb-xl text-center">
        <h1 className="font-headline-lg text-primary mb-sm">Seguimiento de Expediente</h1>
        <p className="text-on-surface-variant text-body-lg">Consulte el estado actual de su solicitud de conciliación.</p>
      </div>
      ...
      <div className="bg-surface-container-lowest p-lg md:p-xl border border-outline-variant rounded-xl shadow-sm mb-xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-outline-variant pb-md mb-md">
          <div>
            <h2 className="font-headline-md text-on-surface mb-xs">{expediente.numero}</h2>
            <p className="text-label-lg font-bold text-primary">{expediente.materia}</p>
          </div>
          <div className="mt-sm md:mt-0 px-md py-xs rounded-full bg-surface-container-high border border-outline-variant font-bold text-label-md text-on-surface">
        ...
        <div className="relative pt-md pb-lg">
          <div className="absolute top-1/2 left-0 w-full h-1 bg-surface-container-highest -translate-y-1/2 rounded-full hidden sm:block"></div>
          <div className="absolute top-1/2 left-0 h-1 bg-primary -translate-y-1/2 rounded-full hidden sm:block transition-all duration-500" style={{ width: `${(step / 4) * 100}%` }}></div>
